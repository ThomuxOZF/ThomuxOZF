...
if (count($_POST) > 0) {
    switch ($mode) {
        case 'enter_email':
            ...
            } elseif (!valid_email($pdo, $mail)) {
                $errors[] = "veillez inserer un email valide";

                 // Envoi d'un courriel de réinitialisation
            } else {
                $_SESSION['forgot']['mail'] = $mail;
                send_email($pdo, $mail);
...
function send_email($pdo, $mail) {
    ...
    send_mail($mail, 'réinitialisation de mot mot de passe', 'votre code est : '. $code);
}

// Fonction de sauvegarde du nouveau mot de passe dans la base de données

function save_password($pdo, $password) {
    $mail = addslashes($_SESSION['forgot']['mail']);
    ...
    switch ($mode) {
        ...
        case 'enter_code':
            ...
            <form method="post" action="forgot.php?mode=enter_code">
                ...
                <br><br>
                <div>
                    <a href="pageaccueil.php">Se connecter</a>
                </div>
            </form>
            <?php
            break;
